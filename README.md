...
> **轻量级项目规划与状态记录工具** - 基于Claude Code交互式命令行，专注于MD规划文档+JSON状态记录

**🎯 核心定位**：
- ✅ **深度需求讨论**：通过结构化对话确保完全理解用户需求
- ✅ **MD规划文档**：生成详细的Markdown规划文档，避免理解偏差
- ✅ **JSON状态记录**：结构化记录执行进度和决策过程
- ✅ **交互式执行**：基于Claude Code命令行的智能执行指导

**🎉 v1.0.0 重大更新**：
- ✅ **轻量化设计**：专注于MD+JSON记录，移除重型执行系统
- ✅ **简化安装流程**：项目级安装，一键配置
- ✅ **完整文档体系**：用户指南、开发者指南、API参考文档

## 🎯 核心特性

### 🤖 深度需求讨论
- **结构化对话流程**：通过Claude Code交互式对话确保完全理解用户需求
- **决策点智能识别**：自动识别项目中的关键决策点和实现细节
- **需求对齐验证**：确保后续执行与用户期望完全一致
- **详细文档生成**：将讨论结果转化为可执行的规划文档

### 📝 MD规划文档系统
- **详细规划输出**：生成包含所有决策和实现细节的Markdown文档
- **避免理解偏差**：作为Claude执行的"永久记忆"，防止理解偏差
- **执行指导**：提供清晰的任务执行顺序和验收标准
- **版本管理友好**：规划文档可纳入版本控制系统

### 📊 JSON状态记录
- **结构化状态管理**：5个核心JSON文件覆盖完整的项目生命周期
- **进度实时跟踪**：记录任务执行状态、质量评分和决策过程
- **状态一致性验证**：定期核对执行结果与规划文档的一致性
- **数据完整性保障**：支持状态备份和恢复机制

### 🔄 交互式执行模式
- **Claude Code原生集成**：基于slash命令的交互式工作流程
- **智能状态提醒**：定期回顾规划文档和当前状态，确保执行一致性
- **灵活执行指导**：根据实际情况调整执行策略，无需严格按计划执行
- **用户全程掌控**：用户始终保持对项目方向和质量的控制权
